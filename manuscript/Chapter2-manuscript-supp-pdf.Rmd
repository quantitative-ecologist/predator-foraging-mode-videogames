...
After removing prey behavior as linear fixed effects (model 2), the percentage of explained variance by the game environments remained almost the same compared to the first model for predator travel speed, the proportion of time spent ambushing, and the proportion of time required to capture the first prey (table SII). Thus, variation in these three behaviors was not explained by differences among game environments. However, there was considerable variation among game environments in the rate of space covered (table SII). The surface area (model 3) of the game environment did not seem to explain a high portion of the variation in space coverage nor in any of the other measured behaviors (table SII).

\vspace{12pt}

\begin{figure}
\centering
\caption*{\textbf{Table SII:} ICC estimates of the additional multivariate mixed-models. The second model excludes prey behavior, and the third model includes the game environment surface area.}
